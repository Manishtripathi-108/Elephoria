        ...
        <div className="group relative rounded-xl border border-light-secondary shadow-neu-light-sm dark:border-dark-secondary dark:shadow-neu-dark-sm">
            ...
            <div className="p-2">
                <h2 className="text-primary line-clamp-1 font-aladin text-sm font-normal capitalize leading-none tracking-widest">
                    {anime.media?.title?.english || 'Untitled'}
                </h2>
                <span className="text-secondary text-xs">
                    {anime.media?.format} &nbsp;&#9679;&nbsp; {anime.media?.episodes} &nbsp;&#9679;&nbsp; {anime.media?.duration} min/ep
                </span>
            </div>
            ...
            <div
                ...
                <p className="text-secondary mb-3 line-clamp-5 font-indie-flower text-sm" aria-live="polite">
                    {anime.media?.description || 'No description available.'}
                </p>

                {/* Additional Info */}
                <div className="text-secondary mb-4 space-y-1 font-indie-flower text-xs">
                    <p>
                        <strong className="text-primary">Japanese: </strong> {anime.media?.title?.native || 'N/A'}
                    </p>
                    <p>
                        <strong className="text-primary">Aired: </strong>
                        {anime.media?.startDate?.day +
                            ' ' +
                            convertMonthNumberToName(anime.media?.startDate?.month) +
                            ' ' +
                            anime.media?.startDate?.year}
                    </p>
                    <p>
                        <strong className="text-primary">Status: </strong> {anime.media?.status || 'Unknown'}
                    </p>
                    <p>
                        <strong className="text-primary">Genres: </strong> {anime.media?.genres?.join(', ') || 'N/A'}
                    </p>
                    <p>
                        <strong className="text-primary">Duration: </strong> {anime.media?.duration} min/ep
                    </p>
                </div>
            </div>
...
export default React.memo(AnimeCards)
