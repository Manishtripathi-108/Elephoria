...
const GameOverModal = ({ initializeGame, playerXName, playerOName, getGameStatus }) => {
    ...
    return (
        <div
            ...
            className="text-secondary absolute left-0 top-0 z-10 flex h-full w-full animate-puff-in cursor-pointer flex-col items-center justify-center gap-8 font-indie-flower">
            {/* Background Overlay */}
            <div className="bg-secondary absolute left-0 top-0 h-full w-full opacity-70 blur-sm saturate-150"></div>

            {/* Display the message */}
            <span className="text-primary z-20 text-6xl font-bold tracking-wider">{message}</span>
