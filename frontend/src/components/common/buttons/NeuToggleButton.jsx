...
const NeuToggleButton = ({ handleClick, active, alignment = 'center', additionalClasses, ...props }) => {
    ...
    return (
        <button
            className={`group flex size-10 shrink-0 grow-0 cursor-pointer flex-col justify-center rounded-lg border border-light-secondary p-2 shadow-neumorphic-xs transition-shadow duration-500 active:shadow-neumorphic-inset-md dark:border-dark-secondary ${alignmentStyles[active ? 'center' : alignment]} ${additionalClasses || ''}`}
            onClick={handleClick}
            {...props}>
            <span
