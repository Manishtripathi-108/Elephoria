...
const getRoomId = (socket) => (callback) => {
    ...
    callback(roomId ? { success: true, roomId } : { success: false, message: 'Unable to create room.' });
};

const handleJoinRoom =
    (socket, io) =>
    ({ roomId, playerName, roomName, isCreateRoom }) => {
        const result = joinRoom(roomId, playerName, roomName, isCreateRoom, socket.id);
        ...
        if (result.success) {
            ...
            socket.emit('gameError', result.message || 'Unable to join the room.');
...
const handleStartGame =
    ...
    ({ roomId }) => {
        ...
        if (result.success) {
            ...
            socket.emit('gameError', result.message || 'Unable to start the game.');
        }
    };
...
const handleBoardChange =
    ...
    ({ roomId, mode }) => {
        ...
        if (result.success) {
            ...
            socket.emit('gameError', result.message || 'Unable to change game mode.');
        }
    };
...
const handleClearBoard = (socket, io) => (roomId) => {
    ...
    if (result.success) {
        ...
        socket.emit('gameError', result.message || 'Unable to clear the board.');
    }
};
...
const handleLeaveRoom = (roomId, socketId, io) => {
    ...
    if (result.success) {
        ...
        io.to(socketId).emit('gameError', result.message || 'Error during room cleanup.');
    }
};

export { getRoomId, handleJoinRoom, handleStartGame, handleMove, handleBoardChange, handleClearBoard, handleLeaveRoom };
