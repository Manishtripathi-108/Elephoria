...
const updateGameState = (movePayload) => {
    ...
    const { mode, isXNext, classicBoard, ultimateBoard, isGameOver, activeIndex } = room;
    ...
    if (mode === 'classic') {
        const updatedBoard = classicBoard.map((cell, i) => (i === macroIndex ? playerSymbol : cell));
        ...
        Object.assign(room, {
            ...
            winner: result.status === 'win' ? room[`player${result.winner}`].name : null,
        ...
        const updatedUltimateBoard = ultimateBoard.map((macro, i) =>
            i === macroIndex ? macro.map((cell, j) => (j === cellIndex ? playerSymbol : cell)) : macro
        );

        const miniBoardStatus = evaluateBoardStatus(updatedUltimateBoard[macroIndex]);
        if (miniBoardStatus.status === 'win') classicBoard[macroIndex] = miniBoardStatus.winner;
        else if (miniBoardStatus.status === 'draw') classicBoard[macroIndex] = 'D';
        ...
        Object.assign(room, {
            ...
            winner: largeBoardStatus.status === 'win' ? room[`player${largeBoardStatus.winner}`].name : null,
            winIndexes: largeBoardStatus.status === 'win' ? largeBoardStatus.line : null,
            isDraw: largeBoardStatus.status === 'draw',
            activeIndex: largeBoardStatus.status === 'continue' && !classicBoard[cellIndex] ? cellIndex : null,
        });

        if (largeBoardStatus.status === 'win') room[`player${largeBoardStatus.winner}`].score++;
...
const leaveRoom = (roomId, socketId) => {
    ...
    if (room.playerX.id === socketId) room.playerX = { id: null, name: null, score: 0 };
    else if (room.playerO.id === socketId) room.playerO = { id: null, name: null, score: 0 };
...
export { generateRoomId, joinRoom, startGame, updateGameState, changeBoard, leaveRoom, clearBoard };
